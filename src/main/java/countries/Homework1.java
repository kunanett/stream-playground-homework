...
public class Homework1 {
    ...
    public boolean streamPipeline1() {
        return countries.stream().map(Country::getName).anyMatch(name -> name.toLowerCase().contains("island"));
    }
    ...
    public void streamPipeline3() {
        countries.stream().map(Country::getName).filter(name -> name.toLowerCase().charAt(0) == name.toLowerCase().charAt(name.length() - 1)).forEach(System.out::println);
    }
    ...
    public void streamPipeline5() {
        countries.stream().sorted(Comparator.comparingLong(Country::getPopulation)).map(Country::getName).limit(10).forEach(System.out::println);
    }
    ...
    public void streamPipeline15() {
        countries.stream().filter(country -> (country.getArea() != null) && (country.getArea().compareTo(BigDecimal.ONE) == -1)).map(Country::getName).forEach(System.out::println);
    }
